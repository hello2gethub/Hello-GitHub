...
char Pub_Buff[256];											//待发送的数据包
unsigned int xj = 0,bz = 0;									
bool change;												//切换
	...
	switch(type)
	{
		...
		case MQTT_PKT_PUBLISH:														//接收的Publish消息
			...
			if(result == 0)
			{
				...
				if(!json)
				{
					UsartPrintf(USART_DEBUG,"解析数据出错:[%s]\n",cJSON_GetErrorPtr());
				}
				else
				{
					json_value = cJSON_GetObjectItem(json,"target");
					UsartPrintf(USART_DEBUG, "下发命令键值：[%s]\n",json_value->string);
					UsartPrintf(USART_DEBUG, "值：[%s]\n",json_value->valuestring);
					if(strstr(json_value->valuestring,"up")!=NULL)					//前进相关
					{
					...
					else if(strstr(json_value->valuestring,"back")!=NULL)			//后退相关
					{
						json_value = cJSON_GetObjectItem(json,"value");
						if(json_value->valueint)
							Car_backward(100);
					...
					}else if(strstr(json_value->valuestring,"left")!=NULL)			//左转相关
					{
						json_value = cJSON_GetObjectItem(json,"value");
						if(json_value->valueint)
							Car_Turn_Left(80);
					...
					}else if(strstr(json_value->valuestring,"right")!=NULL)			//右转相关
					{
						json_value = cJSON_GetObjectItem(json,"value");
						if(json_value->valueint)
							Car_Turn_Right(80);
							...
							Car_Stop(0);
					}else if(strstr(json_value->valuestring,"bz")!=NULL)			//避障相关
					{
						json_value = cJSON_GetObjectItem(json,"value");
						if(json_value->valueint)
						{
							...
							while(1)
							{
								...
								if(WK_UP == 1)
								{
									...
									sprintf(Pub_Buff,"{\"xj\":%d,\"bz\":%d}",xj,bz);	//数据打包
									OneNet_Publish("Topic/pub",Pub_Buff);				//上发至服务器，主题：Topic/pub
									ESP8266_Clear();
									break;
								}
					...
					}else if(strstr(json_value->valuestring,"xj")!=NULL)				//寻迹相关
					{
						json_value = cJSON_GetObjectItem(json,"value");
						if(json_value->valueint)
						{
